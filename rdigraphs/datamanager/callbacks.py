...
# these are initialized by calling function `initialize` with the appropriate
# parameters `dict` (you MUST do it)
...
def initialize(parameters: dict) -> None:
    ...
    organizations_disambiguator = OrganizationsDisambiguator(
        **parameters['organizations'])


def projects_researchers(df: pd.DataFrame, name_col='NOMBRE'):
    """
    ...
    name_col : str, optional
        Name of the colum with the researchers' names
...
def patents_literature(df: pd.DataFrame):
    """
    Callback function for relating patents literature with itself in "patents"
    database.
...
def patents_non_literature(df: pd.DataFrame):
    """
    Callback function for relating patents literature with non-patents
    literature in "patents" database.
...
class Disambiguator:
    ...
    def __init__(self, disambiguation_map: typing.List[str],
                 new_id: str) -> None:
        """
        Initializer.
        ...
        new_id : str
            The name of the new field/column/neo4j property to be created for
            storing the final *disambiguated* id
        """
        ...
        # the *final* mapped-to id is obtained as the concatenation of the
        # (output) "db" and "id" columns...
        self.map[self.new_id] = self.map['std_db'].str.cat(
            self.map['std_id'], sep='_')
        ...
        self.patents_subset = self.map[
            self.map['orig_db'] == self.patents_prefix]
        ...
        self.projects_subset = self.map[
            self.map['orig_db'] == self.projects_prefix]
        self.projects_subset = self.projects_subset.drop(['orig_db'], axis=1)

        # same for "Scopus"
        self.scopus_prefix = 'scps'
        self.scopus_subset = self.map[
            self.map['orig_db'] == self.scopus_prefix]
    ...
    def handle_duplicates(df):
        ...
        unmapped: Pandas dataframe
            Input data with the only row with the value "left_only" in column
            `source`, or the 1st row of the input
    ...
    def merge(self, mapping: pd.DataFrame, df: pd.DataFrame, field: str,
              prefix: str):
        """
        Replace within the passed `DataFrame` the values of `field` that are
        present in the disambiguation map, while at the same time ensuring that
        the new disambiguated values are unique.
        ...
        prefix : str
            Prefix to be added to a "new" identifier built from the already
            existing (not present in the disambiguation map)
        ...
        # lef-join: only the rows in the data (as opposed to those in the
        # disambiguation map) will be present
        merge = df.merge(mapping, how='left', left_on=field,
                         right_on='orig_id', indicator='source')

        # if the value in new column `new_id` is `NaN`, then replace it with
        # that in `field` with `prefix` prepended
        merge[self.new_id] = merge[self.new_id].where(
            merge[self.new_id].notna(), prefix + '_' + merge[field])
        ...
        # the rows which contain duplicates are grouped and *deduplicated* by
        # applying the method `handle_duplicates`
        # NOTE: the index of this `DataFrame` is a `MultiIndex`, but that's not
        # a problem since index is not to be
        # written to the csv file later on
        deduplicated = merge[i_duplicated].groupby(
            self.new_id, group_keys=True).apply(self.handle_duplicates)

        # `unique` and `deduplicated` rows are vertically stacked together
        merge = pd.concat((unique, deduplicated))

        # `orig_id` column (=`field` when there is a match, `NaN` otherwise)
        # is not needed anymore
        merge = merge.drop(['orig_id', 'source'], axis=1)

        return merge

    def replace(self, mapping: pd.DataFrame, df: pd.DataFrame, field: str,
                prefix: str):
        """
        Replace within the passed `DataFrame` the values of `field` that are
        present in the disambiguation map.
        ...
        # TODO: factorize the following code (which is exactly the same as
        # above)
...
class AuthorsDisambiguator(Disambiguator):
    ...
    # NOTE: no subclasses are used for the logic below to avoid duplicating
    # the disambiguation mapping in memory
    ...
    def patstats_person(self, df: pd.DataFrame):
        ...
        # `orig_id` in the disambiguation map is a `str` but `person_id` in
        # "PATSTATS" DB is an `int`
        df['person_id'] = df['person_id'].astype('str')

        return self.merge(self.patents_subset, df, 'person_id',
                          self.patents_prefix)
    ...
    def patstats_person_application(self, df: pd.DataFrame):
        ...
        return self.replace(self.patents_subset, df, 'person_id',
                            self.patents_prefix)
    ...
    def projects_researchers(self, df: pd.DataFrame):
        ...
        return self.merge(self.projects_subset, df, 'NIF',
                          self.projects_prefix)

    def projects_researcher_project(self, df: pd.DataFrame):
        """
        Callback function for "researcher_project" table in "projects"
        database.
        ...
        return self.replace(self.projects_subset, df, 'NIF',
                            self.projects_prefix)
    ...
    def scopus_authorship(self, df: pd.DataFrame):
        ...
        # `orig_id` in the disambiguation map is a `str` but `author_id`
        # in "Scopus" DB is an `int`
        df['author_id'] = df['author_id'].astype('str')

        return self.merge(self.scopus_subset, df, 'author_id',
                          self.scopus_prefix)


# NOTE: this class inherits from `AuthorsDisambiguator` since the methods for
# "patstats" would be the same
class OrganizationsDisambiguator(AuthorsDisambiguator):
    ...
    def projects_organizations(self, df: pd.DataFrame):
        ...
        return self.merge(self.projects_subset, df, 'CIF',
                          self.projects_prefix)
    ...
    def projects_projects(self, df: pd.DataFrame):
        ...
        return self.replace(self.projects_subset, df, 'CIFEMPRESA',
                            self.projects_prefix)
    ...
    def scopus_authorship(self, df: pd.DataFrame):
        ...
        # the id's are converted from `float` (they are `float`because of the
        # missing values) to `str` when they are not `na`
        df['affiliation_id'] = df['affiliation_id'].astype(str).str.rstrip(
            '0.').where(df['affiliation_id'].notna(), 'NA')

        return self.replace(self.scopus_subset, df, 'affiliation_id',
                            self.scopus_prefix)
