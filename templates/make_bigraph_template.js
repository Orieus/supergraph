...
function initialize() {
    ...
    // Here we set our <code>viz</code> variable by instantiating the 
    // <code>vizuly.viz.corona</code> function.
    // All vizuly components require a parent DOM element at initialization so
    // they know where to inject their display elements.
    viz = vizuly.viz.halo_cluster(document.getElementById("viz_container"));

    viz.data(data)
        .width(800).height(600)       // Initial display size
        .haloKey(function (d) {
            return d.SOURCE_CAT; })    // Property that determines each PAC
        .nodeKey(function (d) {
            return d.TARGET_ID; })      // Property that determines Candidate
        .nodeGroupKey(function (d) {
            return d.TARGET_CAT; })      // Property that determines candidate Party affiliation
        .value(function (d) {
            return Number(d.WEIGHT); })   // Pproperty that determines the weight/size of the link path
    ...
    // **Themes and skins** play a big role in vizuly, and are designed to make
    // it easy to make subtle or drastic changes to the look and feel of any
    // component. Here we choose a theme and skin to use for our bar chart.
    // *See this <a href=''>guide</a> for understanding the details of themes
    /// and skins.*
    ...
    // The <code>viz.selection()</code> property refers to the parent container
    // that was used at the object construction.  With this <code>selection</code> property we can use D3
...
// For each mouse over on the node we want to create a datatip that shows
// information about the candidate and any assoicated PACs that have
...
function node_onMouseOver(e,d,i) {
    ...
    createDataTip(x + d.r, y + d.r + 25,
                  ...
                  d.values[0].TARGET_NM, "Total weight: " + total);
}
...
var cx_palette = ["#F80018", "#0543bc", "#F81800", "#0543bc", "#962d62", "#FFa400", "#a4a400", "#F81800", "#FFa400"]
